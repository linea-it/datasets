...
The Photo-z Server (PZ Server) is a web-based platform developed by LIneA as part of Brazil's in-kind contribution to the Rubin Observatory LSST project. It provides access to both official and user-generated data products. The PZ Server administrators maintain and periodically update a curated list of data resources to support the LSST Community with general-purpose examples. 
...
Most data products hosted on the PZ Server will be uploaded or created by users, and there is no quality guarantee for them. Users are encouraged to use the PZ Server to store data products at any stage of development, from experimental samples to refined results associated with scientific publications. 
