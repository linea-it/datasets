...
The Photo-z Server is a web-based platform developed by LIneA as part of Brazil's in-kind contribution to the Rubin Observatory LSST project. It provides access to both official and user-generated data products. The Photo-z Server administrators maintain and periodically update a curated list of data resources to support the LSST Community with general-purpose examples.

This page provides an overview of some datasets available on the Photo-z Server along with access instructions. For more details on the platform's features and functionalities, please refer to the full [**Photo-z Server User Documentation**](https://docs.linea.org.br/en/sci-platforms/pz_server.html).
...
The official datasets will be validated by Rubin's Data Management and will be available to the LSST Community through the Photo-z Server. These datasets will include reference redshift catalogs, training sets, and photo-z estimates associated with the LSST data releases. The datasets will be released in a phased manner, starting with the first data release (DR1) and continuing with subsequent releases as the LSST survey progresses. For now, the [Rubin Observatory PZ Data Products](https://pzserver.linea.org.br/oficial_products) page is empty.

### User-Generated Data Products

Most data products hosted on the Photo-z Server will be uploaded or created by users, and there is no quality guarantee for them. Users are encouraged to use the Photo-z Server to store data products at any stage of development, from experimental samples to refined results associated with scientific publications.

Besides data uploaded by users, the [User-generated Data Products](https://pzserver.linea.org.br/user_products) page hosts a set of datasets that were prepared by the Photo-z Server admins for educational purposes, to serve as use-case examples for the Photo-z Server tutorials. In addition, the DP1 and DP2 data products generated by the PZ Science Unit from Rubin’s Commissioning Team, such as those described in the tech notes [SITCOMTN-154](https://sitcomtn-154.lsst.io/) (for DP1) and [RTN-124](https://rtn-124.lsst.io) (in preparation, for DP2), are also available there. **These data products were only very lightly validated. Treat them as provisional and use them with caution.**
...
A compilation of reference redshifts produced using the pipeline [**Combine Redshift Catalogs (CRC)**](https://docs.linea.org.br/en/sci-platforms/pz_server_crc.html) by combining 38 catalogs with data from 51 surveys, mostly spectroscopic, to be matched to the LSST Object catalog to produce training and test sets for photo-z algorithms.

| Data Product | Description | # of source catalogs | # of objects | Status |
| --- | --- | --- | --- | --- |
| [REF_Z_CLEAN_NO_DESI_LSS](https://pzserver.linea.org.br/product/318_ref_z_clean_no_desi_lss) | Unique objects with quality flag $\geq$ 3, excluding DESI DR1 | 37 |  6,616,173 | :material-check: |
...
| --- | --- | --- | --- |
| [MATCHED_CLEAN RTN-124](https://pzserver.linea.org.br/product/417_matched_clean_rtn124) | [REF_Z_CLEAN_NO_DESI_LSS](https://pzserver.linea.org.br/product/342_ref_z_clean_no_desi_lss) matched to DP2 Objects | 1,194,088 | :material-check: |
| [MATCHED_DESI RTN-124](https://pzserver.linea.org.br/product/418_matched_desi_rtn124) | [DESI DR1 LITE](https://pzserver.linea.org.br/product/314_desi_dr1_lite) matched to DP2 Objects | 1,544,298 | :material-check: |
| [CLIPPED_TRAIN RTN-124 (PARQUET)](https://pzserver.linea.org.br/product/415_clipped_train_rtn124_parquet) | 80% random sample from MATCHED_CLEAN trimmed for biased excess based on i_mag-redshift binning | 554,676 | :material-check: |
| [SOM_TEST RTN-124 (PARQUET)](https://pzserver.linea.org.br/product/412_som_test_rtn124_parquet) | remaining 20% from MATCHED_CLEAN with SOM-based sampling to mimic "DP2 galaxies" | 18,400 | :material-check: |
...
For now, Photo-z Estimates can be accessed via LSDB. For more information, visit the [LSDB.io page](https://data.lsdb.io/Rubin/DP2/object_photoz)
...
Comprehensive Gold dataset for all DP1 fields beyond the fields ECDFS+EDFS+SV_95 and SV_38, where spectroscopic data are available (dataset **not** listed in Table 1 from the tech note [SITCOMTN-154](https://sitcomtn-154.lsst.io/)):
...
Data products containing reference redshift catalogs (before matching with the DP1 Object table) separated by origin survey as listed in Table 2 from the tech note [SITCOMTN-154](https://sitcomtn-154.lsst.io/).
...
On the Photo-z Server, the product type "Training Set" comprises all samples resulting from the matching between a reference redshift and an object catalog. That might include training and test sets together in the same file or independent sub-samples uploaded separately. For the latter, both training and test sets are tagged as "Training Set".
...
Estimator data models are listed in Table 7 and described in Appendix A.3 from the tech note [SITCOMTN-154](https://sitcomtn-154.lsst.io/).
...
As mentioned in Section 3.4 and Appendix A.1 from the tech note [SITCOMTN-154](https://sitcomtn-154.lsst.io/), the configuration files [dp1.yaml](https://github.com/LSSTDESC/rail_project_config/blob/main/dp1/dp1.yaml) (the complete set of configurations tested, labeled as analysis _flavors_) and [dp1_v4.yaml](https://github.com/LSSTDESC/rail_project_config/blob/main/dp1/dp1_v4.yaml) (optimized configuration parameters) are available on the GitHub repository [`rail_project_config`](https://github.com/LSSTDESC/rail_project_config).

### Validation Results

Photo-z point estimates, QP Ensembles, and evaluation metrics are related to the results shown in Table 4. Files were uploaded from directories listed in Table 7 and described in Appendix A.4 from the tech note [SITCOMTN-154](https://sitcomtn-154.lsst.io/).
...
PZ Tables were produced as part of the initial studies with commissioning data described in [SITCOMTN-154](https://sitcomtn-154.lsst.io/). Data were uploaded from the directories listed in Table 7.
...
The Photo-z Server hosts several public spectroscopic redshift catalogs from different surveys that can be used as reference datasets for training and validation of photo-z estimates. The data products listed below are the original catalogs, without any filtering or data selection.
...
Generated by the **[Combine Redshift Catalogs (CRC)](https://docs.linea.org.br/en/sci-platforms/pz_server_crc.html)** pipeline of the **[Photo-z Server](https://docs.linea.org.br/en/sci-platforms/pz_server.html)**.
...
Generated by the **[Training Set Maker (TSM)](https://docs.linea.org.br/en/sci-platforms/pz_server_tsm.html)** pipeline of the **[Photo-z Server](https://docs.linea.org.br/en/sci-platforms/pz_server.html)**.
...
<!--
Quick access via the `pzserver` Python library:
